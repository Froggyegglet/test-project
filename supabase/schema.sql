...
to anon, authenticated
using (verified = true or paid_partner = true);

drop policy if exists "Public can read visible supplier products" on supplier_products;
create policy "Public can read visible supplier products"
...
using (
  exists (
    ...
    where suppliers.id = supplier_products.supplier_id
      and (suppliers.verified = true or suppliers.paid_partner = true)
  )
);
